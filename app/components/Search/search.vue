    ...
    <form ref="referenceRef" role="search" class="relative" @submit.prevent="submit">
      <div class="flex">
        <SfInput ref="inputRef" v-model="inputModel"
          ...
          <template #suffix>
            <button
v-if="inputModel" type="button" aria-label="Reset search"
              class="flex rounded-md focus-visible:outline-solid focus-visible:outline-offset" @click="reset">
              <SfIconCancel />
            </button>
          </template>
        </SfInput>
...
<script setup lang="ts">
  ...
  onMounted(() => {
    // Only initialize MeiliSearch on client-side
    const meilisearchConfig = config.public.meilisearch || {};
    const host = meilisearchConfig.host || 'http://localhost:7700';
    const apiKey = meilisearchConfig.searchApiKey || '';
    
    if (host && host !== 'undefined') {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const client = instantMeiliSearch(host, apiKey);
      // You can use client.searchClient here if needed in the future
    } else {
      console.error('Meilisearch host is not configured properly');
    }
  });

  // Query hook to capture the input query
  const queryHook = (query: string, refine: (arg0: string) => void) => {
