        ...
        <header ref="menuRef"
            ...
            <div class="flex items-center justify-start h-full max-w-[1536px] w-full px-4 md:px-10">
                <SfButton
                    ...
                    :aria-haspopup="true" :aria-expanded="isOpen" variant="tertiary" square @click="toggle()">
                    <SfIconMenu class="text-white" />
                </SfButton>
                <a href="#" aria-label="SF Homepage"
                    class="flex shrink-0 ml-4 md:ml-0 text-white mr-auto md:mr-10 focus-visible:outline-solid focus-visible:outline-offset focus-visible:rounded-sm">
                    <picture>
                        <source
                        ...
                        <img src="https://storage.googleapis.com/sfui_docs_artifacts_bucket_public/production/vsf_logo_sign_white.svg"
                            alt="Sf Logo" class="w-8 h-8 lg:w-50 lg:h-7" />
                    </picture>
                </a>
                <SfButton
                    ...
                    <span class="hidden md:inline-flex whitespace-nowrap px-2">Browse</span>
                </SfButton>
                <nav>
                    <ul>
                        <li role="none">
                            <transition enter-active-class="transform transition duration-500 ease-in-out"
                                ...
                                <SfDrawer ref="drawerRef" v-model="isOpen" disable-click-away placement="top"
                                    class="grid grid-cols-1 md:gap-x-6 md:grid-cols-4 bg-white shadow-lg p-0 max-h-screen overflow-y-auto md:absolute! md:top-20! max-w-[376px] md:max-w-full md:p-6 mr-[50px] md:mr-0">
                                    <div
                                        class="sticky top-0 flex items-center justify-between py-2 px-4 bg-primary-700 md:hidden w-full max-w-[376px]">
                                        <div class="flex items-center typography-text-lg font-medium text-white">Browse
                                        </div>
                                        <SfButton square variant="tertiary" aria-label="Close navigation menu"
                                            class="text-white ml-2" @click="close()" @keydown.enter.space="close()">
                                            <SfIconClose />
                                        </SfButton>
                                    </div>
                                    <div class="nth-2:pt-0 pt-6 md:pt-0">
                                        ...
                                        <hr class="mb-3.5" />
                                        <ul>
                                            <li v-for="child in departments" :key="child?.id">
                                                <SfListItem tag="a" :href="`/departments/${child?.id}`" size="sm"
                                                    role="none"
                                    ...
                                    <div class="nth-2:pt-0 pt-6 md:pt-0">
                                        <h2 role="presentation"
                                            class="typography-text-base font-medium text-neutral-900 whitespace-nowrap p-4 md:py-1.5">
                                            {{ bottomsidebarmenu?.name }}
                                        </h2>
                                        <hr class="mb-3.5" />
                                        <ul>
                                            <li v-for="item in bottomsidebarmenu?.menus" :key="item">
                                                <SfListItem tag="a" :href="item?.url" size="sm"
                                                    role="none"
                                                    class="typography-text-base md:typography-text-sm py-4 md:py-1.5">
                                                    {{ item?.name }}
                                                </SfListItem>
                                    ...
                                    <div class="nth-2:pt-0 pt-6 md:pt-0">
                                        ...
                                        <hr class="mb-3.5" />
                                        <ul>
                                            <li v-for="item in social?.menus" :key="item">
                                                <SfListItem tag="a" :href="item?.url" size="sm"
                                                    role="none"
                                                    class="typography-text-base md:typography-text-sm py-4 md:py-1.5">
                                                    {{ item?.name }}
                                    ...
                                    <div
                                        ...
                                        <img :src="bannerDetails.image" :alt="bannerDetails.title"
                                            class="object-contain" />
                                        <p class="mb-4 mt-4 px-4 text-center typography-text-base font-medium">
                                            {{ bannerDetails.title }}</p>
                                    </div>
